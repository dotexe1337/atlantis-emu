...
if ("$BUILD_NAME" -eq "mainline") {
    $RELEASE_DIST = "atlantis-windows-msvc"
}
else {
    $RELEASE_DIST = "atlantis-windows-msvc-$BUILD_NAME"
}

$MSVC_BUILD_ZIP = "atlantis-windows-msvc-$GITDATE-$GITREV.zip" -replace " ", ""
$MSVC_BUILD_PDB = "atlantis-windows-msvc-$GITDATE-$GITREV-debugsymbols.zip" -replace " ", ""
$MSVC_SEVENZIP = "atlantis-windows-msvc-$GITDATE-$GITREV.7z" -replace " ", ""
$MSVC_TAR = "atlantis-windows-msvc-$GITDATE-$GITREV.tar" -replace " ", ""
$MSVC_TARXZ = "atlantis-windows-msvc-$GITDATE-$GITREV.tar.xz" -replace " ", ""
$MSVC_SOURCE = "atlantis-windows-msvc-source-$GITDATE-$GITREV" -replace " ", ""
...
if ("$env:GITHUB_ACTIONS" -eq "true") {
    ...
    cp .\build\bin\atlantis*.exe .\artifacts\
    ...
    # Debugging symbols
    cp .\build\bin\atlantis*.pdb .\artifacts\
...
rm "$RELEASE_DIST\*.exe"
Get-ChildItem "$BUILD_DIR" -Recurse -Filter "atlantis*.exe" | Copy-Item -destination $RELEASE_DIST
