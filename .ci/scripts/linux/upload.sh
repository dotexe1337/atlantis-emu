...
APPIMAGE_NAME="atlantis-${RELEASE_NAME}-${GITDATE}-${GITREV}.AppImage"
BASE_NAME="atlantis-linux"
...
cp build/bin/atlantis-cmd "$DIR_NAME"
if [ "${RELEASE_NAME}" != "early-access" ] && [ "${RELEASE_NAME}" != "mainline" ]; then
    cp build/bin/atlantis "$DIR_NAME"
fi

# Build an AppImage
cd build

wget -nc https://github.com/atlantis-emu/ext-linux-bin/raw/main/appimage/appimagetool-x86_64.AppImage
...
if [ "${RELEASE_NAME}" = "mainline" ] || [ "${RELEASE_NAME}" = "early-access" ]; then
    echo "X-AppImage-Integrate=false" >> AppDir/org.atlantis_emu.atlantis.desktop
fi

if [ "${RELEASE_NAME}" = "mainline" ]; then
    # Generate update information if releasing to mainline
    ./appimagetool-x86_64.AppImage -u "gh-releases-zsync|atlantis-emu|atlantis-${RELEASE_NAME}|latest|atlantis-*.AppImage.zsync" AppDir "${APPIMAGE_NAME}"
else
...
if [ "${RELEASE_NAME}" = "mainline" ] || [ "${RELEASE_NAME}" = "early-access" ]; then
    cp "build/${APPIMAGE_NAME}" "${DIR_NAME}/atlantis-${RELEASE_NAME}.AppImage"
fi
