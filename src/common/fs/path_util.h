...
enum class atlantisPath {
    atlantisDir,        // Where atlantis stores its data.
    ...
    ScreenshotsDir, // Where atlantis screenshots are stored.
...
/**
 * Gets the filesystem path associated with the atlantisPath enum.
 *
 * @param atlantis_path atlantisPath enum
 *
 * @returns The filesystem path associated with the atlantisPath enum.
 */
[[nodiscard]] const std::filesystem::path& GetatlantisPath(atlantisPath atlantis_path);

/**
 * Gets the filesystem path associated with the atlantisPath enum as a UTF-8 encoded std::string.
 *
 * @param atlantis_path atlantisPath enum
 *
 * @returns The filesystem path associated with the atlantisPath enum as a UTF-8 encoded std::string.
 */
[[nodiscard]] std::string GetatlantisPathString(atlantisPath atlantis_path);

/**
 * Sets a new filesystem path associated with the atlantisPath enum.
 * If the filesystem object at new_path is not a directory, this function will not do anything.
 *
 * @param atlantis_path atlantisPath enum
 * @param new_path New filesystem path
 */
void SetatlantisPath(atlantisPath atlantis_path, const std::filesystem::path& new_path);

#ifdef _WIN32
template <typename Path>
void SetatlantisPath(atlantisPath atlantis_path, const Path& new_path) {
    if constexpr (IsChar<typename Path::value_type>) {
        SetatlantisPath(atlantis_path, ToU8String(new_path));
    } else {
        SetatlantisPath(atlantis_path, std::filesystem::path{new_path});
    }
}
#endif
...
/**
 * Gets the relevant paths for atlantis to store its data based on the given XDG environment variable.
 ...
 *
 * @returns The path where atlantis should store its data.
 */
...
// i.e. "C:\Users\atlantis\Documents\save.bin" becomes {"C:", "Users", "atlantis", "Documents", "save.bin" }
