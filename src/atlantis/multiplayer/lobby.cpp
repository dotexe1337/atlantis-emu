...
#include "atlantis/multiplayer/client_room.h"
#include "atlantis/multiplayer/lobby.h"
#include "atlantis/multiplayer/lobby_p.h"
#include "atlantis/multiplayer/message.h"
#include "atlantis/multiplayer/state.h"
#include "atlantis/multiplayer/validation.h"
...
Lobby::Lobby(QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->nickname->text().isEmpty() || ui->nickname->text() == QStringLiteral("atlantis")) {
        if (!Settings::values.atlantis_username.GetValue().empty()) {
            ui->nickname->setText(
                QString::fromStdString(Settings::values.atlantis_username.GetValue()));
        } else if (!GetProfileUsername().empty()) {
            ui->nickname->setText(QString::fromStdString(GetProfileUsername()));
        } else {
            ui->nickname->setText(QStringLiteral("atlantis"));
        }
    }
...
#ifdef ENABLE_WEB_SERVICE
        if (!Settings::values.atlantis_username.GetValue().empty() &&
            !Settings::values.atlantis_token.GetValue().empty()) {
            WebService::Client client(Settings::values.web_api_url.GetValue(),
                                      Settings::values.atlantis_username.GetValue(),
                                      Settings::values.atlantis_token.GetValue());
