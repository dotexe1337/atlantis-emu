...
#include "atlantis/multiplayer/state.h"
#include "atlantis/util/controller_navigation.h"
...
#include "atlantis/bootmanager.h"
#include "atlantis/compatdb.h"
#include "atlantis/compatibility_list.h"
#include "atlantis/configuration/configure_dialog.h"
#include "atlantis/configuration/configure_input_per_game.h"
#include "atlantis/configuration/qt_config.h"
#include "atlantis/debugger/console.h"
#include "atlantis/debugger/controller.h"
#include "atlantis/debugger/profiler.h"
#include "atlantis/debugger/wait_tree.h"
#include "atlantis/discord.h"
#include "atlantis/game_list.h"
#include "atlantis/game_list_p.h"
#include "atlantis/hotkeys.h"
#include "atlantis/install_dialog.h"
#include "atlantis/loading_screen.h"
#include "atlantis/main.h"
#include "atlantis/play_time_manager.h"
#include "atlantis/startup_checks.h"
#include "atlantis/uisettings.h"
#include "atlantis/util/clickable_label.h"
#include "atlantis/vk_device_info.h"

#ifdef atlantis_CRASH_DUMPS
#include "atlantis/breakpad.h"
#endif

using namespace Common::Literals;

#ifdef USE_DISCORD_PRESENCE
#include "atlantis/discord_impl.h"
#endif
...
void GMainWindow::ShowTelemetryCallout() {
    ...
    const QString telemetry_message =
        tr("<a href='https://atlantis-emu.org/help/feature/telemetry/'>Anonymous "
           "data is collected</a> to help improve atlantis. "
...
static void RemoveCachedContents() {
    const auto cache_dir = Common::FS::GetatlantisPath(Common::FS::atlantisPath::CacheDir);
...
GMainWindow::GMainWindow(std::unique_ptr<QtConfig> config_, bool has_broken_vulkan)
    ...
    LOG_INFO(Frontend, "atlantis Version: {}", atlantis_build_version);
    LogRuntimes();
#ifdef ARCHITECTURE_x86_64
    ...
    if (has_broken_vulkan) {
        ...
        QMessageBox::warning(this, tr("Broken Vulkan Installation Detected"),
                             tr("Vulkan initialization failed during boot.<br><br>Click <a "
                                "href='https://atlantis-emu.org/wiki/faq/"
                                "#atlantis-starts-with-the-error-broken-vulkan-installation-detected'>"
    ...
    //: TRANSLATORS: This string is shown to the user to explain why atlantis needs to prevent the
...
#ifdef atlantis_ENABLE_COMPATIBILITY_REPORTING
...
void GMainWindow::InitializeHotkeys() {
    ...
    LinkActionShortcut(ui->action_Exit, QStringLiteral("Exit atlantis"));
...
void GMainWindow::ConnectMenuEvents() {
    ...
    // Help
    connect_menu(ui->action_Open_atlantis_Folder, &GMainWindow::OnOpenatlantisFolder);
    ...
    if (result == Core::SystemResultStatus::Success &&
        ...
        QMessageBox::warning(
            ...
            tr("You are using the deconstructed ROM directory format for this game, which is an "
               ...
               "support.<br><br>For an explanation of the various Switch formats atlantis supports, <a "
               "href='https://atlantis-emu.org/wiki/overview-of-switch-game-formats'>check out our "
    ...
    if (result != Core::SystemResultStatus::Success) {
        ...
        case Core::SystemResultStatus::ErrorVideoCore:
            QMessageBox::critical(
                ...
                tr("atlantis has encountered an error while running the video core. "
                   "This is usually caused by outdated GPU drivers, including integrated ones. "
                   "Please see the log for more details. "
                   "For more information on accessing the log, please see the following page: "
                   "<a href='https://atlantis-emu.org/help/reference/log-files/'>"
        ...
        default:
            if (result > Core::SystemResultStatus::ErrorLoader) {
                ...
                const auto description =
                    tr("%1<br>Please follow <a href='https://atlantis-emu.org/help/quickstart/'>the "
                       "atlantis quickstart guide</a> to redump your files.<br>You can refer "
                       "to the atlantis wiki</a> or the atlantis Discord</a> for help.",
...
void GMainWindow::BootGame(const QString& filename, Service::AM::FrontendAppletParameters params,
                           StartGameType type) {
    LOG_INFO(Frontend, "atlantis starting...");
...
void GMainWindow::OnGameListOpenFolder(u64 program_id, GameListOpenTarget target,
    ...
    case GameListOpenTarget::SaveData: {
        ...
        const auto nand_dir = Common::FS::GetatlantisPath(Common::FS::atlantisPath::NANDDir);
        auto vfs_nand_dir =
    ...
    case GameListOpenTarget::ModData: {
        ...
        path = Common::FS::GetatlantisPath(Common::FS::atlantisPath::LoadDir) /
...
void GMainWindow::OnTransferableShaderCacheOpenFile(u64 program_id) {
    const auto shader_cache_dir = Common::FS::GetatlantisPath(Common::FS::atlantisPath::ShaderDir);
...
void GMainWindow::OnGameListRemoveInstalledEntry(u64 program_id, InstalledEntryType type) {
    ...
    Common::FS::RemoveDirRecursively(Common::FS::GetatlantisPath(Common::FS::atlantisPath::CacheDir) /
                                     "game_list");
    game_list->PopulateAsync(UISettings::values.game_dirs);
}
...
void GMainWindow::RemoveCustomConfiguration(u64 program_id, const std::string& game_path) {
    ...
    const auto custom_config_file_path =
        Common::FS::GetatlantisPath(Common::FS::atlantisPath::ConfigDir) / "custom" / config_file_name;
...
void GMainWindow::OnGameListDumpRomFS(u64 program_id, const std::string& game_path,
    ...
    const auto dump_dir =
        target == DumpRomFSTarget::Normal
            ? Common::FS::GetatlantisPath(Common::FS::atlantisPath::DumpDir)
            : Common::FS::GetatlantisPath(Common::FS::atlantisPath::SDMCDir) / "atmosphere" / "contents";
...
void GMainWindow::OnGameListNavigateToGamedbEntry(u64 program_id,
    ...
    QDesktopServices::openUrl(QUrl(QStringLiteral("https://atlantis-emu.org/game/") + directory));
}
...
bool GMainWindow::MakeShortcutIcoPath(const u64 program_id, const std::string_view game_file_name,
    ...
    // Get path to atlantis icons directory & icon extension
    std::string ico_extension = "png";
#if defined(_WIN32)
    out_icon_path = Common::FS::GetatlantisPath(Common::FS::atlantisPath::IconsDir);
    ico_extension = "ico";
    ...
    out_icon_path /= (program_id == 0 ? fmt::format("atlantis-{}.{}", game_file_name, ico_extension)
                                      : fmt::format("atlantis-{:016X}.{}", program_id, ico_extension));
    return true;
}

void GMainWindow::OnGameListCreateShortcut(u64 program_id, const std::string& game_path,
                                           GameListShortcutTarget target) {
    // Get path to atlantis executable
    const QStringList args = QApplication::arguments();
    std::filesystem::path atlantis_command = args[0].toStdString();
    // If relative path, make it an absolute path
    if (atlantis_command.c_str()[0] == '.') {
        atlantis_command = Common::FS::GetCurrentDir() / atlantis_command;
    }
    // Shortcut path
    ...
    if (atlantis_command.string().ends_with(appimage_ending) &&
    ...
    const std::string comment = fmt::format("Start {:s} with the atlantis Emulator", game_title);
    const std::string categories = "Game;Emulator;Qt;";
    const std::string keywords = "Switch;Nintendo;";

    if (GMainWindow::CreateShortcutLink(shortcut_path, comment, out_icon_path, atlantis_command,
...
void GMainWindow::OnGameListOpenDirectory(const QString& directory) {
    ...
    if (directory == QStringLiteral("SDMC")) {
        fs_path =
            Common::FS::GetatlantisPath(Common::FS::atlantisPath::SDMCDir) / "Nintendo/Contents/registered";
    } else if (directory == QStringLiteral("UserNAND")) {
        fs_path =
            Common::FS::GetatlantisPath(Common::FS::atlantisPath::NANDDir) / "user/Contents/registered";
    } else if (directory == QStringLiteral("SysNAND")) {
        fs_path =
            Common::FS::GetatlantisPath(Common::FS::atlantisPath::NANDDir) / "system/Contents/registered";
    } else {
...
void GMainWindow::OnMenuReportCompatibility() {
    ...
    if (!Settings::values.atlantis_token.GetValue().empty() &&
        !Settings::values.atlantis_username.GetValue().empty()) {
        CompatDB compatdb{system->TelemetrySession(), this};
        compatdb.exec();
    } else {
        QMessageBox::critical(
            this, tr("Missing atlantis Account"),
            tr("In order to submit a game compatibility test case, you must link your atlantis "
               "account.<br><br/>To link your atlantis account, go to Emulation &gt; Configuration "
               "&gt; "
               "Web."));
    }
#else
...
void GMainWindow::OnOpenModsPage() {
    OpenURL(QUrl(QStringLiteral("https://github.com/atlantis-emu/atlantis/wiki/Switch-Mods")));
}

void GMainWindow::OnOpenQuickstartGuide() {
    OpenURL(QUrl(QStringLiteral("https://atlantis-emu.org/help/quickstart/")));
}

void GMainWindow::OnOpenFAQ() {
    OpenURL(QUrl(QStringLiteral("https://atlantis-emu.org/wiki/faq/")));
}
...
void GMainWindow::OnConfigure() {
    ...
    if (result != QDialog::Accepted && !UISettings::values.configuration_applied &&
        ...
        if (!Common::FS::RemoveDirContentsRecursively(
                Common::FS::GetatlantisPath(Common::FS::atlantisPath::ConfigDir) / "custom")) {
            LOG_WARNING(Frontend, "Failed to remove custom configuration files");
        }
        if (!Common::FS::RemoveDirRecursively(
                Common::FS::GetatlantisPath(Common::FS::atlantisPath::CacheDir) / "game_list")) {
...
void GMainWindow::OnOpenatlantisFolder() {
    QDesktopServices::openUrl(QUrl::fromLocalFile(
        QString::fromStdString(Common::FS::GetatlantisPathString(Common::FS::atlantisPath::atlantisDir))));
}
...
void GMainWindow::OnInstallFirmware() {
    ...
    if (!ContentManager::AreKeysPresent()) {
        QMessageBox::information(
            ...
            tr("Install decryption keys and restart atlantis before attempting to install firmware."));
        return;
    }
        ...
        if (QtProgressCallback(
            ...
            QMessageBox::warning(
                ...
                tr("Firmware installation cancelled, firmware may be in bad state, "
                   "restart atlantis or re-install firmware."));
            return;
        }
    }
...
void GMainWindow::OnInstallDecryptionKeys() {
    ...
    const auto atlantis_keys_dir = Common::FS::GetatlantisPath(Common::FS::atlantisPath::KeysDir);
    for (auto key_file : source_key_files) {
        std::filesystem::path destination_key_file = atlantis_keys_dir / key_file.filename();
...
void GMainWindow::OnCaptureScreenshot() {
    ...
    const auto screenshot_path =
        QString::fromStdString(Common::FS::GetatlantisPathString(Common::FS::atlantisPath::ScreenshotsDir));
    const auto date =
...
void GMainWindow::MigrateConfigFiles() {
    const auto config_dir_fs_path = Common::FS::GetatlantisPath(Common::FS::atlantisPath::ConfigDir);
...
void GMainWindow::UpdateWindowTitle(std::string_view title_name, std::string_view title_version,
    ...
    const auto atlantis_title = fmt::format("atlantis | {}-{}", branch_name, description);
    const auto override_title =
        fmt::format(fmt::runtime(std::string(Common::g_title_bar_format_idle)), build_id);
    const auto window_title = override_title.empty() ? atlantis_title : override_title;
    ...
    if (!ContentManager::AreKeysPresent()) {
        QMessageBox::warning(
            ...
            tr("Encryption keys are missing. "
               "<br>Please follow <a href='https://atlantis-emu.org/help/quickstart/'>the atlantis "
...
bool GMainWindow::ConfirmClose() {
    ...
    const auto text = tr("Are you sure you want to close atlantis?");
    return question(this, tr("atlantis"), text);
}
...
bool GMainWindow::ConfirmForceLockedExit() {
    ...
    const auto text = tr("The currently running application has requested atlantis to not exit.\n\n"
...
int main(int argc, char* argv[]) {
    ...
    QCoreApplication::setOrganizationName(QStringLiteral("atlantis team"));
    QCoreApplication::setApplicationName(QStringLiteral("atlantis"));
    ...
    // suffix.
    QGuiApplication::setDesktopFileName(QStringLiteral("org.atlantis_emu.atlantis"));
#endif
