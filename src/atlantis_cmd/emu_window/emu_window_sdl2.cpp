...
#include "atlantis_cmd/emu_window/emu_window_sdl2.h"
#include "atlantis_cmd/atlantis_icon.h"
...
void EmuWindow_SDL2::WaitEvent() {
    ...
    if (current_time > last_time + 2000) {
        ...
        const auto title =
            fmt::format("atlantis {} | {}-{} | FPS: {:.0f} ({:.0f}%)", Common::g_build_fullname,
...
void EmuWindow_SDL2::SetWindowIcon() {
    SDL_RWops* const atlantis_icon_stream = SDL_RWFromConstMem((void*)atlantis_icon, atlantis_icon_size);
    if (atlantis_icon_stream == nullptr) {
        LOG_WARNING(Frontend, "Failed to create atlantis icon stream.");
        return;
    }
    SDL_Surface* const window_icon = SDL_LoadBMP_RW(atlantis_icon_stream, 1);
