...
import org.atlantis.atlantis_emu.fragments.CoreErrorDialogFragment
...
import org.atlantis.atlantis_emu.model.Patch
import org.atlantis.atlantis_emu.model.GameVerificationResult

/**
 * Class which contains methods that interact
 * with the native side of the atlantis code.
 */
object NativeLibrary {
    ...
    init {
        try {
            System.loadLibrary("atlantis-android")
    ...
    fun openContentUri(path: String?, openmode: String?): Int {
        return if (DocumentsTree.isNativePath(path!!)) {
            atlantisApplication.documentsTree!!.openContentUri(path, openmode)
        } else {
    ...
    fun getSize(path: String?): Long {
        return if (DocumentsTree.isNativePath(path!!)) {
            atlantisApplication.documentsTree!!.getFileSize(path)
        } else {
    ...
    fun exists(path: String?): Boolean {
        return if (DocumentsTree.isNativePath(path!!)) {
            atlantisApplication.documentsTree!!.exists(path)
        } else {
    ...
    fun isDirectory(path: String?): Boolean {
        return if (DocumentsTree.isNativePath(path!!)) {
            atlantisApplication.documentsTree!!.isDirectory(path)
        } else {
    ...
    fun getParentDirectory(path: String): String =
        if (DocumentsTree.isNativePath(path)) {
            atlantisApplication.documentsTree!!.getParentDirectory(path)
        } else {
            path
        }
    ...
    fun getFilename(path: String): String =
        if (DocumentsTree.isNativePath(path)) {
            atlantisApplication.documentsTree!!.getFilename(path)
        } else {
    ...
    /**
     * Logs the atlantis version, Android version and, CPU.
     */
