...
object GameIconUtils {
    private val imageLoader = ImageLoader.Builder(atlantisApplication.appContext)
        .components {
            add(GameIconKeyer())
            add(GameIconFetcher.Factory())
        }
        .memoryCache {
            MemoryCache.Builder(atlantisApplication.appContext)
                .maxSizePercent(0.25)
                .build()
        }
        .build()
    ...
    fun loadGameIcon(game: Game, imageView: ImageView) {
        val request = ImageRequest.Builder(atlantisApplication.appContext)
            .data(game)
            .target(imageView)
    ...
    suspend fun getShortcutIcon(lifecycleOwner: LifecycleOwner, game: Game): IconCompat {
        val layerDrawable = ResourcesCompat.getDrawable(
            atlantisApplication.appContext.resources,
            R.drawable.shortcut,
            null
        ) as LayerDrawable
        layerDrawable.setDrawableByLayerId(
            R.id.shortcut_foreground,
            getGameIcon(lifecycleOwner, game).toDrawable(atlantisApplication.appContext.resources)
        )
        val inset = atlantisApplication.appContext.resources
