...
import org.atlantis.atlantis_emu.databinding.CardGameBinding
...
class GameAdapter(private val activity: AppCompatActivity) :
    ...
    inner class GameViewHolder(val binding: CardGameBinding) :
        ...
        fun onClick(game: Game) {
            ...
            val preferences =
                PreferenceManager.getDefaultSharedPreferences(atlantisApplication.appContext)
            preferences.edit()
                .putLong(
            ...
            activity.lifecycleScope.launch {
                withContext(Dispatchers.IO) {
                    val shortcut =
                        ShortcutInfoCompat.Builder(atlantisApplication.appContext, game.path)
                            .setShortLabel(game.title)
                            .setIcon(GameIconUtils.getShortcutIcon(activity, game))
                            .setIntent(game.launchIntent)
                            .build()
                    ShortcutManagerCompat.pushDynamicShortcut(atlantisApplication.appContext, shortcut)
                }
            }
