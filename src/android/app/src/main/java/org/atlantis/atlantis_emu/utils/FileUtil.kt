...
import org.atlantis.atlantis_emu.model.MinimalDocumentFile
...
object FileUtil {
    ...
    fun getFilename(uri: Uri): String {
        val resolver = atlantisApplication.appContext.contentResolver
        val columns = arrayOf(
            DocumentsContract.Document.COLUMN_DISPLAY_NAME
        )
        var filename = ""
    ...
    fun DocumentFile.copyFilesTo(
        ...
        this.listFiles().forEach {
            ...
            if (it.isDirectory) {
                newFile.mkdirs()
                DocumentFile.fromTreeUri(atlantisApplication.appContext, it.uri)?.copyFilesTo(newFile)
            } else {
                val inputStream =
                    atlantisApplication.appContext.contentResolver.openInputStream(it.uri)
    ...
    fun DocumentFile.inputStream(): InputStream =
        atlantisApplication.appContext.contentResolver.openInputStream(uri)!!

    fun DocumentFile.outputStream(): OutputStream =
        atlantisApplication.appContext.contentResolver.openOutputStream(uri)!!

    fun Uri.inputStream(): InputStream =
        atlantisApplication.appContext.contentResolver.openInputStream(this)!!

    fun Uri.outputStream(): OutputStream =
        atlantisApplication.appContext.contentResolver.openOutputStream(this)!!

    fun Uri.asDocumentFile(): DocumentFile? =
        DocumentFile.fromSingleUri(atlantisApplication.appContext, this)
}
