...
std::optional<NcaID> RegisteredCache::GetNcaIDFromMetadata(u64 title_id,
    ...
    const auto res1 = CheckMapForContentRecord(atlantis_meta, title_id, type);
    if (res1)
        return res1;
...
void RegisteredCache::AccumulateatlantisMeta() {
    const auto meta_dir = dir->GetSubdirectory("atlantis_meta");
    if (meta_dir == nullptr) {
        return;
    }
    ...
    for (const auto& file : meta_dir->GetFiles()) {
        ...
        CNMT cnmt(file);
        atlantis_meta.insert_or_assign(cnmt.GetTitleID(), std::move(cnmt));
    }
}
...
std::optional<u32> RegisteredCache::GetEntryVersion(u64 title_id) const {
    ...
    const auto atlantis_meta_iter = atlantis_meta.find(title_id);
    if (atlantis_meta_iter != atlantis_meta.cend()) {
        return atlantis_meta_iter->second.GetTitleVersion();
    }
...
void RegisteredCache::IterateAllMetadata(
    ...
    for (const auto& kv : atlantis_meta) {
        const auto& cnmt = kv.second;
        for (const auto& rec : cnmt.GetContentRecords()) {
            if (GetFileAtID(rec.nca_id) != nullptr && filter(cnmt, rec)) {
                out.push_back(proc(cnmt, rec));
...
InstallResult RegisteredCache::InstallEntry(const NCA& nca, TitleType type,
    ...
    if (!RawInstallatlantisMeta(new_cnmt)) {
        return InstallResult::ErrorMetaFailed;
    }
...
bool RegisteredCache::RemoveExistingEntry(u64 title_id) const {
    ...
    // If patch entries for any program exist in atlantis meta, remove them
    for (u8 i = 0; i < 0x10; i++) {
        const auto meta_dir = dir->CreateDirectoryRelative("atlantis_meta");
...
bool RegisteredCache::RawInstallatlantisMeta(const CNMT& cnmt) {
    ...
    Refresh();
    return std::find_if(atlantis_meta.begin(), atlantis_meta.end(),
