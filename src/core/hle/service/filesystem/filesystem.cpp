...
std::shared_ptr<FileSys::SaveDataFactory> FileSystemController::CreateSaveDataFactory(
    ...
    using atlantisPath = Common::FS::atlantisPath;
    const auto rw_mode = FileSys::OpenMode::ReadWrite;

    auto vfs = system.GetFilesystem();
    const auto nand_directory =
        vfs->OpenDirectory(Common::FS::GetatlantisPathString(atlantisPath::NANDDir), rw_mode);
...
void FileSystemController::CreateFactories(FileSys::VfsFilesystem& vfs, bool overwrite) {
    ...
    using atlantisPath = Common::FS::atlantisPath;
    const auto sdmc_dir_path = Common::FS::GetatlantisPath(atlantisPath::SDMCDir);
    ...
    auto load_directory = vfs.OpenDirectory(Common::FS::GetatlantisPathString(atlantisPath::LoadDir),
                                            FileSys::OpenMode::Read);
    auto sd_load_directory = vfs.OpenDirectory(Common::FS::PathToUTF8String(sdmc_load_dir_path),
                                               FileSys::OpenMode::Read);
    auto dump_directory =
        vfs.OpenDirectory(Common::FS::GetatlantisPathString(atlantisPath::DumpDir), rw_mode);
