...
void SinkStream::AppendBuffer(SinkBuffer& buffer, std::span<s16> samples) {
    ...
    auto atlantis_volume{Settings::Volume()};
    if (atlantis_volume > 1.0f) {
        atlantis_volume = 0.6f + 20 * std::log10(atlantis_volume);
    }
    auto volume{system_volume * device_volume * atlantis_volume};
