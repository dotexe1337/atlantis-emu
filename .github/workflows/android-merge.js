...
async function tagAndPushEA(github, owner, repo, execa) {
    ...
    console.log(`New tag: ${newTag}`);
    console.info('Pushing tags to GitHub ...');
    await execa("git", ["remote", "add", "android", "https://github.com/atlantis-emu/atlantis-android.git"]);
...
async function mergePullRequests(pulls, execa) {
    ...
    await execa("git", ["config", "--global", "user.name", "atlantisbot"]);
    await execa("git", [
        "config",
        "--global",
        "user.email",
        "atlantis\x40atlantis-emu\x2eorg", // prevent email harvesters from scraping the address
    ]);
    let hasFailed = false;
    for (let pull of pulls) {
        let pr = pull.number;
        ...
        try {
            ...
            const process2 = execa("git", ["commit", "-m", `Merge atlantis-emu#${pr}`]);
...
async function resetBranch(execa) {
    ...
    let hasFailed = false;
    try {
        await execa("git", ["remote", "add", "source", "https://github.com/atlantis-emu/atlantis.git"]);
...
async function getMainlineTag(execa) {
    ...
    let hasFailed = false;
    try {
        await execa("git", ["remote", "add", "mainline", "https://github.com/atlantis-emu/atlantis-android.git"]);
...
async function mergebot(github, context, execa) {
    // Reset our local copy of master to what appears on atlantis-emu/atlantis - master
    ...
    await fetchPullRequests(pulls, "https://github.com/atlantis-emu/atlantis", execa);
    const mergeResults = await mergePullRequests(pulls, execa);

    if (BUILD_EA) {
        await tagAndPushEA(github, 'atlantis-emu', `atlantis-android`, execa);
    } else {
        await generateReadme(pulls, context, mergeResults, execa);
        await tagAndPush(github, 'atlantis-emu', `atlantis-android`, execa, true);
    }
}
